...
export default class TestCommon extends TerminalTestSuite {
  ...
  async test_renew_database() {
    ...
    let expectedExpirationDate = new Date();
    expectedExpirationDate.setDate(expectedExpirationDate.getDate() + 90);
    expectedExpirationDate = expectedExpirationDate.toISOString().slice(0, 10)
    const actualExpirationDate = new Date(expirationParamWithUuid.value).toISOString().slice(0, 10);
    this.assertTrue(
      actualExpirationDate === expectedExpirationDate,
      `expiration_date should be as expected ${expectedExpirationDate} != ${actualExpirationDate}`
    );
